...
{	
	...
	public class MapManager extends Sprite
	{
		...
		public function MapManager( stref:Stage )
		{
			_stref = stref;
		}
		public function generateElf():void {
			...
			if ( _elfArmy != null ) {
				...
				_elfArmy.y = santa.y + 12;
				addChild( _elfArmy );
			}
			_elfArmy.elfIdle();
			_elfArmy.addEventListener(GameEvent.ON_ELF_IDLE_DONE, onElfIdleDone );
		}
		private function onElfIdleDone(e:GameEvent):void 
		{
		...
		private function onElfWalk(e:Event):void 
		{
			...
			if( lvlManager.houseArray[currHouseIndex].x + 50 >= _elfArmy.x ) {
		...
		public function addGlobalListener():void {
			...
			this.addEventListener(GameEvent.ON_CURRPOINT_UPDATE, onCurrPointUpdate);
			this.addEventListener(GameEvent.ON_BONUS_CLICK, onBtnBonusClick );
			
			
			//this.addEventListener(GameEvent.ON_ELF_IDLE_DONE, onElfIdleDone );
		}
		...
		private function onCurrPointUpdate(e:GameEvent):void 
		{ 	
			//this send values to popWindow
			popUpManager.setText( 2011, e.params.points );
		...
		private function removeListener():void
		{
			...
			this.removeEventListener(GameEvent.ON_CURRPOINT_UPDATE, onCurrPointUpdate);
			this.removeEventListener(GameEvent.ON_BONUS_CLICK, onBtnBonusClick );
			//this.removeEventListener(GameEvent.ON_ELF_IDLE_DONE, onElfIdleDone );
		}
