package Interactive 
{
	...
	public class SantaClaus extends Sprite 
	{
		...
		{
			trace(this,"timerJump:",e.target.currentCount );
			if ( e.target.currentCount > 2 ) {
				if( this.parent != null ){
				this.parent.dispatchEvent( new GameEvent( GameEvent.ON_SANTA_JUMP_DONE ));
			}
				timerForjump.stop();
				timerForjump.reset();
				removeSantaInvremoveSantaEvent();
			}
		}
		public function removeSantaInvremoveSantaEvent():void {
			timerForjump.removeEventListener( TimerEvent.TIMER, onTimer );
		}
		public function santaCalling():void {
			santa.gotoAndStop(4);
		}
