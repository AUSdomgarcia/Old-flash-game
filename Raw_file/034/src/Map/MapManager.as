...
{	
	...
	{
		//CharData
		private var charData:CharaterData = new CharaterData();
		...
		public function init():void
		{
			...
			if ( charData.charLevel == 0) {
				charData.charLevel = 6;
				lvlNum = charData.charLevel;
			}
		}
		// LLL - Listeners
			...
			if ( _santaInv.isOnGamePlay ) {
				...
				for (var i:int = 0; i < lvlManager.houseArray[currHouseIndex].heldDreamListArr.length; i++)
				{
					...
					!lvlManager.houseArray[currHouseIndex].lightIsOn && !lvlManager.houseArray[currHouseIndex].chimneyIsOn  ) {
						...
						//if ( lvlManager.houseArray[currHouseIndex].lightIsOn && lvlManager.houseArray[currHouseIndex].chimneyIsOn ) {
						 //e.params.need != lvlManager.houseArray[currHouseIndex].heldDreamListArr[i] &&
						 //lvlManager.houseArray[currHouseIndex].dreamListHolderpArr[i].isVisible == false &&
