...
{	
	...
	public class MapManager extends Sprite
	{
		...
		//ItemList & Inventory
		private var pickMC:PickSantaMC = new PickSantaMC();
		//Meter
		...
		{	
			...
			trace("@@@@@===========================", lvlManager.houseArray[currHouseIndex].lightIsOn );
			trace("@@@@@===========================", lvlManager.houseArray[currHouseIndex].chimneyIsOn );
		...
		private function calllvlManagerHouse(e:GameEvent):void 
		{
			...
			var genRandItemArr:Array = generatedRandomItemForHouse;
			rManager.setInvQtyAndGeneRandItem( _santaInv.refArrForInvIcon , genRandItemArr );
		}
		...
		private function removeSanta():void {
			if ( santa.parent != null ) {
				...
				removeChild( santa );
			} // santa have automatic listener remover
		}
		private function removePickSanta():void {
			if ( pickMC.parent != null ) {
			removeChild(pickMC);
			removeChild( meter );
			}
		}
